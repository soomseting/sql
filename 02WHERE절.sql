...
--------------------------------------------------------------------------------
-- ORDER BY 구문
SELECT * FROM EMPLOYEES ORDER BY SALARY; -- ASC가 DEFAULT = 오름차순
SELECT * FROM EMPLOYEES ORDER BY SALARY DESC; -- 내림차순
SELECT * FROM EMPLOYEES ORDER BY DEPARTMENT_ID, SALARY DESC; -- 먼저 부서 아이디로 정렬, 동순위에 대해 SALARY 내림차순 정렬 
SELECT FIRST_NAME, SALARY * 12 AS 연봉 FROM EMPLOYEES ORDER BY 연봉 DESC; -- 엘리어스를 정렬해 사용할 수 있음.
SELECT FIRST_NAME, SALRAY * 12 AS 연봉 FROM EMPLOYEES WHERE 연봉 >= 10000; -- WHERE 절이 SELECT 보다 먼저 동작해서 에러 뜸
-- WHERE 사용 X
